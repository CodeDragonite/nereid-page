...
import { upcomingProjects, type UpcomingProject } from "@/lib/upcoming";
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: "1200px", margin: "0 auto", padding: "5rem 1.5rem 4rem", width: "100%" }}>
        ...
        >
          What We&apos;re Building{" "}
          <em style={{ fontStyle: "italic", color: "var(--accent)" }}>Next</em>
        </motion.h1>
        <motion.p
          custom={2}
          initial={reduced ? "visible" : "hidden"}
        ...
        >
          Exploring new frontiers in healthcare technology and sports analytics. Join the waitlist and be first to know when we launch.
        </motion.p>
...
function ProjectCard({ project, index }: { project: UpcomingProject; index: number }) {
  ...
  const reduced = useReducedMotion();
  const Icon = project.category === "healthcare" ? Heart : Zap;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          <span style={{
            ...
            background: `color-mix(in srgb, ${project.color} 20%, transparent)`,
            color: project.color,
          }}>
            {project.phase}
          </span>
      ...
      <h2 style={{ fontSize: "1.5rem", fontWeight: 600, color: "var(--text-primary)", marginBottom: "0.75rem", lineHeight: "1.3" }}>
        {project.title}
      </h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem", marginBottom: "1.5rem" }}>
        {project.features.map((feature) => (
          <div key={feature} style={{ display: "flex", alignItems: "flex-start", gap: "6px" }}>
            <CheckCircle size={14} style={{ color: project.color, flexShrink: 0, marginTop: "2px" }} aria-hidden="true" />
            <span style={{ fontSize: "0.8rem", color: "var(--text-secondary)", lineHeight: "1.4" }}>{feature}</span>
          </div>
        ))}
      </div>
      ...
      <Link
        ...
        onMouseEnter={(e) => {
          const el = e.currentTarget as HTMLElement;
          el.style.background = `color-mix(in srgb, ${project.color} 30%, transparent)`;
          el.style.transform = "translateY(-1px)";
        }}
        onMouseLeave={(e) => {
          const el = e.currentTarget as HTMLElement;
          el.style.background = `color-mix(in srgb, ${project.color} 20%, transparent)`;
          el.style.transform = "translateY(0)";
        }}
        aria-label={`${project.ctaText} for ${project.title}`}
      >
        {project.ctaText}
    ...
    <main>
      ...
      >
        ...
        >
          ...
          <h2 id="projects-heading" style={{ fontFamily: "var(--font-display)", fontSize: "clamp(1.75rem, 3vw, 2.5rem)", color: "var(--text-primary)", letterSpacing: "-0.02em" }}>
