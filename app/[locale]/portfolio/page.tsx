...
function PortfolioHero() {
  ...
  return (
    <section
      ...
      style={{ position: "relative", minHeight: "60vh", display: "flex", alignItems: "center", overflow: "hidden", paddingTop: "68px" }}
    >
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: "1200px", margin: "0 auto", padding: "5rem 1.5rem 4rem", width: "100%" }}>
        <motion.div custom={0} initial={reduced ? "visible" : "hidden"} animate="visible" variants={fadeUp} style={{ marginBottom: "1.5rem" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: "5px 12px", borderRadius: "20px", fontSize: "11px", fontWeight: "600", letterSpacing: "0.1em", textTransform: "uppercase" as const, background: "var(--accent-dim)", color: "var(--accent)", border: "1px solid var(--border-hover)" }}>
            <Target size={11} aria-hidden="true" /> {t("heroTag")}
          </span>
        </motion.div>
        <motion.h1 id="portfolio-heading" custom={1} initial={reduced ? "visible" : "hidden"} animate="visible" variants={fadeUp} style={{ fontFamily: "var(--font-display)", fontSize: "clamp(2.5rem, 6vw, 5rem)", lineHeight: "1.05", color: "var(--text-primary)", marginBottom: "1.5rem", maxWidth: "820px", letterSpacing: "-0.02em" }}>
          {t("heroHeadline")}{" "}
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: "1rem" }}>
        <span style={{ display: "inline-block", padding: "3px 10px", borderRadius: "20px", fontSize: "0.7rem", fontWeight: "600", letterSpacing: "0.08em", textTransform: "uppercase" as const, background: `color-mix(in srgb, ${study.color} 15%, transparent)`, color: study.color }}>
          {study.category}
        </span>
      </div>
      <h3 style={{ fontSize: "1.25rem", fontWeight: 600, color: "var(--text-primary)", marginBottom: "0.5rem", lineHeight: "1.3" }}>{study.title}</h3>
      <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginBottom: "1rem" }}>{study.client}</p>
      <p style={{ fontSize: "0.875rem", color: "var(--text-secondary)", lineHeight: "1.6", marginBottom: "0.75rem" }}>
        <strong style={{ color: "var(--text-primary)", fontWeight: 500 }}>{t("challenge")}: </strong>
        {study.challenge.length > 120 ? study.challenge.slice(0, 120) + "…" : study.challenge}
      </p>
      <p style={{ fontSize: "0.875rem", color: study.color, lineHeight: "1.6", marginBottom: "1rem", fontWeight: 500 }}>↑ {study.outcome}</p>
      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.375rem", marginBottom: "1.25rem", marginTop: "auto" }}>
        {study.tech.map((t) => (
          <span key={t} style={{ padding: "2px 8px", borderRadius: "4px", fontSize: "0.7rem", background: `color-mix(in srgb, ${study.color} 10%, transparent)`, color: "var(--text-secondary)" }}>{t}</span>
        ))}
      </div>
      <Link href={`/portfolio/${study.slug}`} style={{ display: "inline-flex", alignItems: "center", gap: "6px", color: study.color, textDecoration: "none", fontSize: "0.875rem", fontWeight: 600, transition: "gap 0.2s ease" }}
        onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.gap = "10px"; }}
        onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.gap = "6px"; }}
        aria-label={`${t("viewCaseStudy")}: ${study.title}`}
      >
        {t("viewCaseStudy")} <ExternalLink size={14} aria-hidden="true" />
      </Link>
    </motion.article>
  );
}
...
function CaseStudiesGrid() {
  ...
  return (
    <section aria-labelledby="case-studies-heading" style={{ padding: "4rem 1.5rem 6rem", maxWidth: "1200px", margin: "0 auto" }}>
      <motion.div ref={headRef} initial={reduced ? { opacity: 1 } : { opacity: 0, y: 20 }} animate={headInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} style={{ textAlign: "center", marginBottom: "3rem" }}>
        ...
        <h2 id="case-studies-heading" style={{ fontFamily: "var(--font-display)", fontSize: "clamp(1.75rem, 3vw, 2.5rem)", color: "var(--text-primary)", letterSpacing: "-0.02em" }}>{t("heading")}</h2>
      </motion.div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "1.25rem" }}>
        {caseStudies.map((study, i) => <CaseStudyCard key={study.id} study={study} index={i} />)}
      </div>
    </section>
  );
}
